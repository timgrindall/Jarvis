...
class JarvisEarpiece:
    def __init__(self, server_url="http://localhost:5000"):
        ...
        self.audio_frames = []
        self.active = False
        self.space_released = threading.Event()  # Signals spacebar was released
    ...
    def do_voice_session(self):
        """
        Starts a temporary pynput listener ONLY for this recording session.
        Hold SPACE to record, release to send. ESC to cancel.
        Listener is destroyed as soon as the session ends.
        """
        ...
        def on_release(key):
            if key == keyboard.Key.space:
                self.space_released.set()
                return False  # Stop listener after first release

        with keyboard.Listener(on_press=on_press, on_release=on_release) as listener:
            self.space_released.wait()  # Block until space released or ESC
            listener.stop()
    ...
    def run(self):
        while True:
            ...
            if cmd == "quit":
                ...
                # Start a short-lived pynput session just for this recording
                ...
                print("  V = voice  |  T = text  |  S = standby  |  quit = exit")
