...
class JarvisEarpiece:
    ...
    def do_text_session(self):
        """Loops taking text input until ESC or blank line."""
        print("Text mode active. Blank line or ESC to go to standby.\n")
        while True:
            ...
            if not text or "\x1b" in text:
                self._go_standby()
                return
            thread = threading.Thread(target=self.process_text_command, args=(text,))
            thread.daemon = True
            thread.start()
            thread.join()  # Wait for response before next prompt
